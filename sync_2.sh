...
default_load_from="best_ade.pth"
...
while true; do  
    for config in "${configs[@]}"; do
        ...
        if $first && $resume; then
            ...
            CUDA_VISIBLE_DEVICES=2,3 PORT=30010bash tools/dist_train.sh "$config" 2 --work-dir "$WORK_DIR" --resume --cfg-options load_from="$resume_from" randomness.diff_rank_seed=False randomness.seed=1539460459 &
            first=false
        else
            echo "Starting training from $load_from for $config"
            CUDA_VISIBLE_DEVICES=2,3 PORT=30010 bash tools/dist_train.sh "$config" 2 --work-dir "$WORK_DIR" --cfg-options load_from="$load_from" randomness.diff_rank_seed=False randomness.seed=1539460459 &
        fi
