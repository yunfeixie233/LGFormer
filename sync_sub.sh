...
check_gpu_memory() {
    ...
    # --format=csv,nounits - 以CSV格式输出，不显示单位
    local mem_usage=$(nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits)

    # 将内存使用量分割成数组
    local mem_array=($mem_usage)

    # 检查每个GPU的内存使用情况
    for usage in "${mem_array[@]}"; do
        if [ $usage -ge 1000 ]; then
            # 如果内存使用量大于或等于1000M，返回1
            return 1
        fi
    done

    # 如果所有GPU的内存使用量都小于1000M，返回0
    return 0
}
...
default_load_from="best_stem.pth"
...
configs=(
    "/data2/yunfei/SpformerV1/configs/superformer/pascalpart/part/gt_extra_small_pre_p9p10p11_nols_avg4_1loss_convstem.py"
)
should_skip_sync() {
