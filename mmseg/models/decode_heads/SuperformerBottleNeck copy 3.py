...
class GroupingBlock(nn.Module):
    ...
    def forward(self, x, group_tokens, return_attn=False):
        ...
        x = self.norm_x(x) #x是pixel feature, B,784,384 (784=28*28 patch)
        # [B, S_2, C]
...
class GroupingLayer(nn.Module):
    ...
    def forward(self, x, prev_group_token=None, return_attn=False):
        ...
        if self.group_token_init_methods == "learnable":
        ...
        elif self.group_token_init_methods == "lift_avgpool":
            b, n, c = x.shape
            h_w = int(math.sqrt(n))
            ...
            group_token = None
        B, L, C = x.shape
        cat_x = self.concat_x(x, group_token)# grouop_token 和 x concat起来过若干个vit block
...
class SuperformerBottleNeck(BaseDecodeHead):
    def __init__(
        ...
        group_token_init_methods: str = "learnable",
        **kwargs
...
**kwargs)
        ...
        #different kinds of feature merging for hypercolumn 
        if self.trip_fuse:
        ...
        for i_layer in range(self.num_merges):
            ...
            layer = GroupingLayer(
                ...
                sp_sizes=sp_sizes,
                group_token_init_methods=group_token_init_methods,
    ...
    def forward_features(
        ...
        for i, stage in enumerate(self.stages):
            ...
            if i < self.num_merges:
                sp_features, group_token, attn_dict = self.mergelayers[i](
                    sp_features_seg, group_token, return_attn=False
                )
