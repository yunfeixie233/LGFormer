...
class SuperformerBottleNeck_ori_mask(MaskFormerHead):
    def __init__(
        self,
...
**kwargs)
        ...
        if self.seg_specific_classifier:
            ...
            if self.classification_feature in ['superpixel','superpixel_bilinear','superpixel_similarity',"superpixel_extralayer",'superpixel_extralayer_similarity']:
                ...
                self.seg_norm = norm_layer(self.embed_dim)
            elif self.classification_feature == 'mask_extralayer':
                self.seg_head = nn.Linear(self.embed_dim, self.part_seg_num_classes)    
                self.seg_norm = norm_layer(self.embed_dim)
    ...
    def forward_segmentation(
        ...
        final_group = gt_list[-1]
        all_cls_scores = self.cls_embed(final_group)
        ...
        all_cls_scores = all_cls_scores.unsqueeze(0)
        all_mask_preds = all_mask_preds.unsqueeze(0)
    ...
    def forward(
        ...
        if generate_seg:
            ...
            all_cls_scores, all_mask_preds = self.forward_segmentation(
