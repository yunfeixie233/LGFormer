...
class LightGroupAttnBlock(nn.Module):
    def __init__(self,
        ...
        if query_shape and key_shape:
            self.query_pos_embed = nn.Parameter(torch.zeros(1,query_shape, query_dims))
            self.key_pos_embed = nn.Parameter(torch.zeros(1,key_shape, key_dims))
        else:
    ...
    def forward(self, query, key, value, att_bias=None, attn_dict_list=None):
        def _inner_forward(query, key, value, att_bias,attn_dict_list = None):
            if self.query_pos_embed is not None and self.key_pos_embed is not None:
                query = query + self.query_pos_embed
                key = key + self.key_pos_embed
                value = value + self.key_pos_embed
            q = self.norm_query(query)
            k = q if self.key_is_query else self.norm_key(key)
            v = k if self.value_is_key else self.norm_value(value)
...
class GPBlock(nn.Module):
    def __init__(self,
        ...
        _group_att_cfg = dict(
            ...
            key_dims=pixel_dims,
            value_dims=gt_dims,
            query_shape = num_group_token,
            key_shape = pixel_shape,
    ...
    def forward(self, pixel, superpixel, hw_shape, attn_dict_list = None, prev_token = None,):
        ...
        elif self.group_projector_methonds == "cross" or "group":
            gt = group_token
        else:
        ...
        elif self.group_projector_methonds == "group" and prev_token is not None:
